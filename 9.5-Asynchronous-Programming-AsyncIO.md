...
sequenceDiagram
    ...
    Loop->>Task1: Start download
    activate Task1
    Task1-->>Loop: Awaiting network (pauses & yields)
    deactivate Task1

    Loop->>Task2: Start download
    activate Task2
    Task2-->>Loop: Awaiting network (pauses & yields)
    deactivate Task2

    Loop->>Task3: Start download
    activate Task3
    Task3-->>Loop: Awaiting network (pauses & yields)
    deactivate Task3

    Note over Loop: All 3 downloads run in background simultaneously!

    Task1-->>Loop: Network response ready!
    activate Task1
    Task1-->>Loop: Done File A
    deactivate Task1

    Task2-->>Loop: Network response ready!
    activate Task2
    Task2-->>Loop: Done File B
    deactivate Task2

    Task3-->>Loop: Network response ready!
    activate Task3
    Task3-->>Loop: Done File C
    deactivate Task3

    Note over Loop: Total Time: ~2.00 seconds!
```
...
flowchart TD
    ...
    E -- Awaits I/O --> F[Register I/O with OS Poller & Suspend Task]
    E -- Finished --> G[Store Result / Clean up Task]
...
sequenceDiagram
    ...
    Note over Thread,Loop: asyncio.run(main()) starts
    Thread->>Loop: Initialize Event Loop
    Loop->>TaskA: Schedule & Start Task A
    activate TaskA
    TaskA->>OS: Register socket read (wait 2s)
    TaskA-->>Loop: await (Yield control)
    deactivate TaskA

    Loop->>TaskB: Schedule & Start Task B
    activate TaskB
    TaskB->>OS: Register socket read (wait 1s)
    TaskB-->>Loop: await (Yield control)
    deactivate TaskB

    Note over Loop,OS: [1.0s passes] OS notifies Event Loop: Task B socket is ready!
    Loop->>TaskB: Resume Task B
    activate TaskB
    TaskB-->>Loop: Task B completes and returns result
    deactivate TaskB

    Note over Loop,OS: [Another 1.0s passes] OS notifies Event Loop: Task A socket is ready!
    Loop->>TaskA: Resume Task A
    activate TaskA
    TaskA-->>Loop: Task A completes and returns result
    deactivate TaskA

    Loop->>Thread: All tasks done. Return to caller.
```
